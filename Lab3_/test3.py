...
def fish(x, y, k):
	"""function is drawing a fish(body is an ellipse, eyes are two circles, flippers are a number of polygons)
	 x - координата центра тела рыбы по х
 y - координата центра рыбы по y
 k - длина рыбы увеличенная в 80 раз"""
...
def Bird(x, y, k):
	"""рисуем птицу: сперва тело как набор эллипсов, потом хвост, крылья, клюв и когти как многоугольники
	 x - координата центра тела рыбы по х
 y - координата центра рыбы по y
 k - длина крыла увеличенная в 130 раз"""
...
def albatross1(x, y, k):
 ...
 x - координата центра тела рыбы по х
 y - координата центра рыбы по y
 k - длина крыла увеличенная в 60 раз"""
...
def albatross2(x, y, k):
	"""чайка второго типа(горизонтальная). Две дуги + кружочек по центру
 x - координата центра тела рыбы по х
 y - координата центра рыбы по y
 k - длина крыла увеличенная в 60 раз"""
...
def albatross3(x, y, k):
	"""чайка третьего типа(наклонённая вправо). Две дуги + кружочек по центру
	x - координата центра тела рыбы по х
	y - координата центра рыбы по y
	k - длина крыла увеличенная в 60 раз"""
